...
import { flashcardCreateSchema, flashcardBatchCreateSchema } from '../../../lib/schemas/flashcard.schema';
import type { ApiErrorResponse, ApiResponse } from '../../../lib/schemas/flashcard.schema';
import { createFlashcard, createFlashcardBatch } from '../../../lib/services/flashcard.service';
import type { FlashcardResponseSchema, FlashcardBatchResponseSchema } from '../../../lib/schemas/flashcard.schema';
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  console.log(`[${requestId}] Processing flashcard creation request`);
  ...
  // 4. Determine if this is a batch request or single flashcard
  const isBatch = Array.isArray(requestBody.flashcardList);
  ...
  try {
    if (isBatch) {
      ...
      console.log(`[${requestId}] Successfully created ${result.listTotal} flashcards`);
      ...
      if (!validationResult.success) {
        console.error(`[${requestId}] Validation failed for flashcard data:`, validationResult.error);
        return createErrorResponse(
          400,
          'INVALID_INPUT.VALIDATION_FAILED',
          'Invalid flashcard data provided',
          { errors: validationResult.error.format() }
        );
      }

      // Create single flashcard
      const result = await createFlashcard(supabase, validationResult.data, userId);
      
      console.log(`[${requestId}] Successfully created flashcard with ID: ${result.flashcardId}`);
      
      return createSuccessResponse<FlashcardResponseSchema>(201, result);
    }
  } catch (error) {
    ...
    console.error(
      `[${requestId}] Flashcard creation failed: ${error instanceof Error ? error.message : String(error)}`
    );
